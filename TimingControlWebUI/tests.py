    ...
    def test_periodically_add_runs(self):
        ...
        with open('team_entry_data.csv') as csvfile:
            ...
            for row in readCSV:
                ...
                r = RunOrder(team_name=row[0], tag=row[2], car_number=row[3], cones=0 if row[4] == '' else int(row[4]), off_courses=0 if row[5] == '' else int(row[5]),  dnfs=0 if row[6] == '' else int(row[6]),  raw_time=0 if row[7] == '' else row[7], adjusted_time=float_input(row[8]))
                db.session.add(r)
                db.session.commit()
