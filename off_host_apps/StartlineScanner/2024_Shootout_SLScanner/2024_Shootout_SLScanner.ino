...
const char *ID             = "slscan";                     // Name of our device, must be unique
    ...
    if(QueueHandle != NULL){ // Sanity check just to make sure the queue actually exists
      ...
      if(ret == pdPASS){
        ...
        if(data != StoredScannedTagValue && data.length() == 17)
        {
          ...
          //REMOVE FOR FLSCANNER
          const char *cameraTriggerPubData = "A";
          if (!client.publish(PUB_TOPIC_1.c_str(), cameraTriggerPubData)) { Serial.println("Message failed to send: " + String(PUB_TOPIC_1)); }
          Serial.println("Message Send : " + String(PUB_TOPIC_1) + " => " + String(cameraTriggerPubData));
